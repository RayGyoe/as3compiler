...
|------|----------:|-------:|
| C (cc -O2) | 50 | 1.00× |
| JavaScript (Node v24) | 62 | 1.24× |
| **as3compiler** (→ C → cc -O2) | 50 | 1.00× |
| 原生 AS3 (AIR/mxmlc) | 689 | 13.78× |

## 分析
- **as3compiler 1.00×**：`Vector.<Number>` 单态化为连续 `double` 数组，索引读写生成
  带边界检查的直接内存访问；与 C 数组几乎一致。
- **原生 AS3 13.78×** 是本集合中差距最大的一项：AVM2 的 `Vector` 索引每次都走
